...
export class App {
  ...
  constructor() {
    ...
    this.bridge.init({
      onSyncDone: () => this.completeSync(),
      onSyncSummaryNeedsUpdate: () => this.loadSyncSummary(),
      onLastSyncLabelNeedsUpdate: () => this.loadLastSyncLabel(),
    });
    this.checkForUpdate();
  ...
  protected syncMissingRoutes(): void {
    ...
    this.syncSummary.set(null);
    this.resetCounters();
    this.dataRefresh.startSync('Syncing missing routes...');
    const c = (globalThis as any).chrome;
    if (c?.tabs?.create) {
  ...
  protected async clearAndResync(): Promise<void> {
    ...
    if (!confirmed) { return; }
    this.dataRefresh.startSync('Clearing synced data...');
    await Promise.all([
    ...
    this.resetCounters();
    this.bridge.setPendingHistoryTrigger('clear_and_resync');
